...
mydb=mysql.connector.connect(host='localhost',user='root',password='JYOSRItha@04',db='music_streaming')
Session(app)
@app.route('/')
def welcome():
        ...
        if mydb.is_connected():
            ...
            if request.method=="POST":
                ...
                if new_artist_2:
                    ...
                    artist_image_2 = request.files['artist_image_2']
                    ...
                    if existing_artist:
                        ...
                        art_id = genotp()
                        filename=artist_image_2.filename
                        
                        cursor.execute('insert into artists (artist_name,created_by,artist_pic) values (%s,%s,%s) ',[new_artist_2,session['creater'],filename])
                        path=os.path.join(os.path.abspath(os.path.dirname(__file__)),'static')
                        if not os.path.exists(path):
                            os.makedirs(path,exist_ok=True)
                
                        artist_image_2.save(os.path.join(path, filename))
                        mydb.commit()
                ...
                x  = cursor.execute('INSERT INTO songs (song_name,release_date, audio_data, song_picture,mood,album_id, created_by,uploaded_at,artist_id,director_id,singer_id,artist2,singer2) VALUES (%s, %s, %s, %s,%s,%s, %s, %s,%s,%s, %s,%s,%s)',
                            (song_name,release_year, song_name+audio_id, song_name+song_id,mood_str,album_name if request.form['album_name'] else n_album_id, session['creater'],present_date,artist_name if request.form['artist_name'] else n_artist_id,Director if request.form['Director'] else n_director_id,singers if request.form['singers'] else n_singer_id,artist_name_2 if request.form['artist_name_2'] else n_artist_id2,singers_2 if request.form['singers_2'] else n_singer_id_2))
...
def updatesong(song_id):
    if session.get('creater'):
        ...
        if request.method=="POST":
            ...
            print(artist1_id)
            cursor.execute('update artists set  artist_name=%s where created_by=%s and artist_id=%s',(artist_name,session['creater'],artist1_id))
            mydb.commit()
            cursor.execute('SELECT singer_id FROM songs WHERE song_id = %s and created_by=%s', (song_id,session['creater'])) 

            singer1_id = cursor.fetchone()[0]
            cursor.execute('update singers set singer_name=%s where created_by=%s and singer_id=%s',(singer_name,session['creater'],singer1_id))
            if singer2 !=None:
                cursor.execute('SELECT singer2 FROM songs WHERE  song_id= %s and created_by=%s', (song_id,session['creater'])) 
                singer2_id = cursor.fetchone()[0]
                cursor.execute('update singers set singer_name=%s where created_by=%s and singer_id=%s',(singer2,session['creater'],singer2_id))
                mydb.commit()
            if artist2!=None:
                cursor.execute('SELECT artist2 FROM songs WHERE  song_id= %s and created_by=%s', (song_id,session['creater'])) 
                artist2_id = cursor.fetchone()[0]
                print(artist2_id)
                cursor.execute('update artists set  artist_name=%s where created_by=%s and artist_id=%s',(artist2,session['creater'],artist2_id)) 
                mydb.commit()   
            ...
            mydb.commit()
            flash('songs updated sucessfully!')
            return redirect(url_for('creater_all_songs'))
